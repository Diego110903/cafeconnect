    ...
    Ajax({
        ...
        fSuccess: (Resp) => {
            if (Resp.code == 200) {
                ...
                $div.innerHTML = `<label for="mediopago">MedioPago</label><select class="form-select" name="mediopago" id="mediopago" required><option value="">Seleccione uno</option>${opc}</select>`;
            }
        }
    });
}